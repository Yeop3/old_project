...
/**
 * Class DriverTaxiOrdersListHandler.
 */
final class DriverTaxiOrdersListHandler implements BotHandler
{
    ...
    public function execute(BotMan $botMan, Bot $botModel, ?string ...$params): void
    {
        ...
        if (!$orders->count()) {
            $botMan->reply('На данный момент нет заказов на доставку');
        ...
        $ordersTexts = $orders->map(function (Order $order): string {
            return "<b>Заказ {$order->getFullText()}</b>\n"
                ...
                ."Обработать заказ - /order_delivery_process_{$order->number}";
        });
