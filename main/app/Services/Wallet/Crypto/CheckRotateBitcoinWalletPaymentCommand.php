...
use App\Services\Wallet\Bitcoin\GetBitcoinWalletInfoFromBlockCypher;
...
/**
 * Class CheckRotateBitcoinWalletPaymentCommand.
 */
final class CheckRotateBitcoinWalletPaymentCommand
{
    private GetBitcoinWalletInfoFromBlockCypher $getBitcoinWalletInfoFromBlockCypher;
    private OrderPaymentHandler $orderPaymentHandler;

    public function __construct(GetBitcoinWalletInfoFromBlockCypher $getBitcoinWalletInfoFromBlockCypher, OrderPaymentHandler $orderPaymentHandler)
    {
        $this->getBitcoinWalletInfoFromBlockCypher = $getBitcoinWalletInfoFromBlockCypher;
    ...
    public function execute(Order $order): void
    {
        ...
        if ($order->wallet->currency->getValue() !== 'btc') {
            return;
        }
        ...
        try {
            $walletInfo = $this->getBitcoinWalletInfoFromBlockCypher->execute($order->wallet->address, $order->wallet->confirmations, $order->wallet->proxy);
    ...
    {
        $amount = $incomingTransaction['value'];
